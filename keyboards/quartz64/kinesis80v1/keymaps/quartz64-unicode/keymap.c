...
*/
[_MC] = LAYOUT(
    VRSN,    _______, _______, _______, _______, _______,   EMAIL1,  MC4,     PWD4,    PWD3,    PWD2,    PWD1,
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
  switch (keycode) {
      ...
      if (record->event.pressed) {
        send_unicode_hex_string("U+00A0");
        send_unicode_string("—");
    ...
    case MC4:
      if (record->event.pressed) {
        SEND_STRING("Ujnjdj" SS_DOWN(X_LCTL) SS_TAP(X_ENTER) SS_UP(X_LCTL));
      }
      return false;
      break;
